...
import { ProductPayload } from "~/services/createProduct";
...
const EditProductPage = () => {
  ...
  const getProductData = async (productID: number) => {
    try {
      ...
      const { data } = res;
      if (data.result) {
        const _product = data.products[0] as ProductDetail;
  ...
  useEffect(() => {
    ...
    getProductData(Number(product_id));
  }, [product_id]);

  useEffect(() => {
    if (!currentProduct) return;
    setReleaseName(currentProduct.name);
    setDescription(currentProduct.description);
    setPrice(currentProduct.price);
    setGenre(currentProduct.genre);
    setReleaseDate(currentProduct.release_date);
    setRecordLabel(currentProduct.label);
    setFeaturedArtists(currentProduct.artist);
    setFileName(currentProduct.filename);
    setArtworkPreview(currentProduct.artwork_cache);
  }, [currentProduct]);
  ...
  const handleSubmit = async () => {
    ...
    if (releaseArtwork) {
      payload.artwork = releaseArtwork.name;
    }
    if (downloadableTrackFile) {
      payload.download = downloadableTrackFile.name;
    }
