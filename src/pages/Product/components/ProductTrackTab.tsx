...
import { setProductTrackData, ProductPayload } from "~/services/createProduct";
...
const ProductTrackTab = ({ postToken }: Props) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await setProductTrackData(payload, postToken);
      if (res.data?.result) {
        toast({
...
export default ProductTrackTab;
1;
